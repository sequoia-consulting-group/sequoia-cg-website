...
var Sakura: any = function (
  ...
  function randomArrayElem(arr: any[]) {
  ...
  this.createPetal = function () {
    ...
    var animationsArr = [
      ...
      ""
        ...
        .concat(
          ((fallTime > 30 ? fallTime : 30) - 20 + randomInt(0, 20)).toString(),
    ...
    petal.style.borderRadius = ""
      ...
      .concat(randomInt(1, Math.floor(width / 4)).toString(), "px");
    petal.style.height = "".concat(height.toString(), "px");
    petal.style.left = "".concat(
      (Math.random() * document.documentElement.clientWidth - 100).toString(),
      "px",
    );
    petal.style.marginTop = "".concat(
      (-(Math.floor(Math.random() * 20) + 15)).toString(),
      "px",
    );
...
interface BlossomProps {
  startBlossom: boolean;
}

const Blossom = ({ startBlossom }: BlossomProps) => {
  useEffect(() => {
    if (startBlossom) {
